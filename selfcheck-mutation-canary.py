...
MUTATIONS: list[tuple[str, str, str, str]] = [
    ...
    ("governed-outcomes-check.py", "misfiled-handoff unlabelled fallback removed",
     "        elif body_ids:", "        elif False:"),
    ("governed-outcomes-check.py", "misfiled-handoff read truncated to 4KB (known false-negative)",
     'head = f.read_text(errors="replace")\n', 'head = f.read_text(errors="replace")[:4000]\n'),
    ("governed-outcomes-check.py", "heartbeat staleness comparison disabled",
     "elif (now - last[name]).total_seconds() > cad * 3600 * 2:", "elif False:"),
    ("governed-outcomes-check.py", "never-seen checks promoted to adverse (cries wolf on fresh installs)",
     '"adverse": len(stale),', '"adverse": len(stale)+len(never),'),
